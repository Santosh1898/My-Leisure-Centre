...
@Component({
  selector: 'app-centre-tidworth',
  templateUrl: './centre-tidworth.page.html',
  styleUrls: ['./centre-tidworth.page.scss'],
})
export class CentreTidworthPage implements OnInit {
  selectedDate: { [key: string]: Date } = {}; 
  selectedDay: { [key: string]: string } = {};
  selectedTime: { [key: string]: string } = {};
  timeSlots: any[] = [];
  dateList: Date[] = [];
  activities_tidworth: any[] = [];
...
// Transfering to Payment page
  ...
  ngOnInit(): void {
    this.firestoreService.getActivitiesTidworth().subscribe(
      (data: any) => {
        this.activities_tidworth = data;
        console.log('Activity:', this.activities_tidworth);

        // Initialize selectedDate for each activity
        for (const activity of this.activities_tidworth) {
  ...
  selectDate(date: Date, activity: any) {
    ...
    this.firestoreService
      .getTimeSlotsForTidworth(activity.id, this.selectedDay[activity.id])
      .subscribe(
        (timeSlotsData) => {
          if (timeSlotsData) {
            const index = this.activities_tidworth.findIndex(
              (item) => item.id === activity.id
            );
            if (index !== -1) {
              const timeSlots = Object.values(timeSlotsData);
            // Sorting the time slots in ascending order
            this.activities_tidworth[index].timeSlots = timeSlots.sort();
            }
          } else {
            const index = this.activities_tidworth.findIndex(
              (item) => item.id === activity.id
            ...
            if (index !== -1) {
              this.activities_tidworth[index].timeSlots = [];
            }
          }
        },
        (error) => {
  ...
  bookNow() {
    if (!this.storedSelectedDate || !this.selectedActivityTimeSlot) {
      ...
      this.router.navigate(['payment'], {
        ...
        queryParams: {
          centreName: "Tidworth Leisure Centre",
