...
@Component({
  selector: 'app-centre-castle-place',
  templateUrl: './centre-castle-place.page.html',
  styleUrls: ['./centre-castle-place.page.scss'],
})
export class CentreCastlePlacePage implements OnInit {
  selectedDate: { [key: string]: Date } = {}; 
  selectedDay: { [key: string]: string } = {};
  selectedTime: { [key: string]: string } = {};
  timeSlots: any[] = [];
  dateList: Date[] = [];
  activities_castlePlace: any[] = [];
  ...
  ngOnInit(): void {
    this.firestoreService.getActivitiesCastlePlace().subscribe(
      (data: any) => {
        this.activities_castlePlace = data;
        console.log('Activity:', this.activities_castlePlace);

        // Initialize selectedDate for each activity
        for (const activity of this.activities_castlePlace) {
  ...
  selectDate(date: Date, activity: any) {
    ...
    this.firestoreService
      .getTimeSlotsForCastlePlace(activity.id, this.selectedDay[activity.id])
      .subscribe(
        (timeSlotsData) => {
          if (timeSlotsData) {
            const index = this.activities_castlePlace.findIndex(
              (item) => item.id === activity.id
            );
            if (index !== -1) {
              const timeSlots = Object.values(timeSlotsData);
            // Sorting the time slots in ascending order
            this.activities_castlePlace[index].timeSlots = timeSlots.sort();
            }
          } else {
            const index = this.activities_castlePlace.findIndex(
              (item) => item.id === activity.id
            ...
            if (index !== -1) {
              this.activities_castlePlace[index].timeSlots = [];
            }
          }
        },
        (error) => {
  ...
  bookNow() {
    if (!this.storedSelectedDate || !this.selectedActivityTimeSlot) {
      ...
      this.router.navigate(['payment'], {
        ...
        queryParams: {
          centreName: "Castle Place Leisure Centre",
