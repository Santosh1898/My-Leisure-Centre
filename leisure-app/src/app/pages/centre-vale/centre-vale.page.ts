...
@Component({
  selector: 'app-centre-vale',
  templateUrl: './centre-vale.page.html',
  styleUrls: ['./centre-vale.page.scss'],
})
export class CentreValePage implements OnInit {
...
// Transfering to Payment page
  ...
  ngOnInit(): void {
    this.firestoreService.getActivitiesVale().subscribe(
      (data: any) => {
        this.activities_vale = data;
        console.log('Activity:', this.activities_vale);

        // Initialize selectedDate for each activity
        for (const activity of this.activities_vale) {
  ...
  selectDate(date: Date, activity: any) {
    ...
    this.firestoreService
      .getTimeSlotsForVale(activity.id, this.selectedDay[activity.id])
      .subscribe(
        (timeSlotsData) => {
          if (timeSlotsData) {
            const index = this.activities_vale.findIndex(
              (item) => item.id === activity.id
            );
            if (index !== -1) {
              const timeSlots = Object.values(timeSlotsData);
            // Sorting the time slots in ascending order
            this.activities_vale[index].timeSlots = timeSlots.sort();
            }
          } else {
            const index = this.activities_vale.findIndex(
              (item) => item.id === activity.id
            ...
            if (index !== -1) {
              this.activities_vale[index].timeSlots = [];
            }
          }
        },
        (error) => {
  ...
  bookNow() {
    if (!this.storedSelectedDate || !this.selectedActivityTimeSlot) {
      ...
      this.router.navigate(['payment'], {
        ...
        queryParams: {
          centreName: "The Vale Community Campus",
