...
@Component({
  selector: 'app-centre-olympiad',
  templateUrl: './centre-olympiad.page.html',
  styleUrls: ['./centre-olympiad.page.scss'],
})
export class CentreOlympiadPage implements OnInit {
  selectedDate: { [key: string]: Date } = {}; 
  selectedDay: { [key: string]: string } = {};
  selectedTime: { [key: string]: string } = {};
  timeSlots: any[] = [];
  dateList: Date[] = [];
  activities_olympiad: any[] = [];
...
// Transfering to Payment page
  ...
  ngOnInit(): void {
    this.firestoreService.getActivitiesOlympiad().subscribe(
      (data: any) => {
        this.activities_olympiad = data;
        console.log('Activity:', this.activities_olympiad);

        // Initialize selectedDate for each activity
        for (const activity of this.activities_olympiad) {
  ...
  selectDate(date: Date, activity: any) {
    ...
    this.firestoreService
      .getTimeSlotsForOlympiad(activity.id, this.selectedDay[activity.id])
      .subscribe(
        (timeSlotsData) => {
          if (timeSlotsData) {
            const index = this.activities_olympiad.findIndex(
              (item) => item.id === activity.id
            );
            if (index !== -1) {
              const timeSlots = Object.values(timeSlotsData);
            // Sorting the time slots in ascending order
            this.activities_olympiad[index].timeSlots = timeSlots.sort();
            }
          } else {
            const index = this.activities_olympiad.findIndex(
              (item) => item.id === activity.id
            ...
            if (index !== -1) {
              this.activities_olympiad[index].timeSlots = [];
            }
          }
        },
        (error) => {
  ...
  bookNow() {
    if (!this.storedSelectedDate || !this.selectedActivityTimeSlot) {
      ...
      this.router.navigate(['payment'], {
        ...
        queryParams: {
          centreName: "Olympiad Leisure Centre",
