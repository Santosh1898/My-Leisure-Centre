...
@Component({
  selector: 'app-centre-melksham',
  templateUrl: './centre-melksham.page.html',
  styleUrls: ['./centre-melksham.page.scss'],
})
export class CentreMelkshamPage implements OnInit {
  selectedDate: { [key: string]: Date } = {}; 
  selectedDay: { [key: string]: string } = {};
  selectedTime: { [key: string]: string } = {};
  timeSlots: any[] = [];
  dateList: Date[] = [];
  activities_melksham: any[] = [];
...
// Transfering to Payment page
  ...
  ngOnInit(): void {
    this.firestoreService.getActivitiesMelksham().subscribe(
      (data: any) => {
        this.activities_melksham = data;
        console.log('Activity:', this.activities_melksham);

        // Initialize selectedDate for each activity
        for (const activity of this.activities_melksham) {
  ...
  selectDate(date: Date, activity: any) {
    ...
    this.firestoreService
      .getTimeSlotsForMelksham(activity.id, this.selectedDay[activity.id])
      .subscribe(
        (timeSlotsData) => {
          if (timeSlotsData) {
            const index = this.activities_melksham.findIndex(
              (item) => item.id === activity.id
            );
            if (index !== -1) {
              const timeSlots = Object.values(timeSlotsData);
            // Sorting the time slots in ascending order
            this.activities_melksham[index].timeSlots = timeSlots.sort();
            }
          } else {
            const index = this.activities_melksham.findIndex(
              (item) => item.id === activity.id
            ...
            if (index !== -1) {
              this.activities_melksham[index].timeSlots = [];
            }
          }
        },
        (error) => {
  ...
  bookNow() {
    if (!this.storedSelectedDate || !this.selectedActivityTimeSlot) {
      ...
      this.router.navigate(['payment'], {
        ...
        queryParams: {
          centreName: "Melksham Community Campus",
