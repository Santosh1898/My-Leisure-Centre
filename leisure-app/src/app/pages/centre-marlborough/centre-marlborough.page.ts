...
@Component({
  selector: 'app-centre-marlborough',
  templateUrl: './centre-marlborough.page.html',
  styleUrls: ['./centre-marlborough.page.scss'],
})
export class CentreMarlboroughPage implements OnInit {
  selectedDate: { [key: string]: Date } = {}; 
  selectedDay: { [key: string]: string } = {};
  selectedTime: { [key: string]: string } = {};
  timeSlots: any[] = [];
  dateList: Date[] = [];
  activities_marlborough: any[] = [];
...
// Transfering to Payment page
  ...
  ngOnInit(): void {
    this.firestoreService.getActivitiesMarlborough().subscribe(
      (data: any) => {
        this.activities_marlborough = data;
        console.log('Activity:', this.activities_marlborough);

        // Initialize selectedDate for each activity
        for (const activity of this.activities_marlborough) {
  ...
  selectDate(date: Date, activity: any) {
    ...
    this.firestoreService
      .getTimeSlotsForMarlborough(activity.id, this.selectedDay[activity.id])
      .subscribe(
        (timeSlotsData) => {
          if (timeSlotsData) {
            const index = this.activities_marlborough.findIndex(
              (item) => item.id === activity.id
            );
            if (index !== -1) {
              const timeSlots = Object.values(timeSlotsData);
            // Sorting the time slots in ascending order
            this.activities_marlborough[index].timeSlots = timeSlots.sort();
            }
          } else {
            const index = this.activities_marlborough.findIndex(
              (item) => item.id === activity.id
            ...
            if (index !== -1) {
              this.activities_marlborough[index].timeSlots = [];
            }
          }
        },
        (error) => {
  ...
  bookNow() {
    if (!this.storedSelectedDate || !this.selectedActivityTimeSlot) {
      ...
      this.router.navigate(['payment'], {
        ...
        queryParams: {
          centreName: "Marlborough Leisure Centre",
