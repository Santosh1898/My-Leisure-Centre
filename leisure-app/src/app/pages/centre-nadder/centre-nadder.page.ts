...
@Component({
  selector: 'app-centre-nadder',
  templateUrl: './centre-nadder.page.html',
  styleUrls: ['./centre-nadder.page.scss'],
})
export class CentreNadderPage implements OnInit {
  selectedDate: { [key: string]: Date } = {}; 
  selectedDay: { [key: string]: string } = {};
  selectedTime: { [key: string]: string } = {};
  timeSlots: any[] = [];
  dateList: Date[] = [];
  activities_nadder: any[] = [];
...
// Transfering to Payment page
  ...
  ngOnInit(): void {
    this.firestoreService.getActivitiesNadder().subscribe(
      (data: any) => {
        this.activities_nadder = data;
        console.log('Activity:', this.activities_nadder);

        // Initialize selectedDate for each activity
        for (const activity of this.activities_nadder) {
  ...
  selectDate(date: Date, activity: any) {
    ...
    this.firestoreService
      .getTimeSlotsForNadder(activity.id, this.selectedDay[activity.id])
      .subscribe(
        (timeSlotsData) => {
          if (timeSlotsData) {
            const index = this.activities_nadder.findIndex(
              (item) => item.id === activity.id
            );
            if (index !== -1) {
              const timeSlots = Object.values(timeSlotsData);
            // Sorting the time slots in ascending order
            this.activities_nadder[index].timeSlots = timeSlots.sort();
            }
          } else {
            const index = this.activities_nadder.findIndex(
              (item) => item.id === activity.id
            ...
            if (index !== -1) {
              this.activities_nadder[index].timeSlots = [];
            }
          }
        },
        (error) => {
