...
@Component({
  selector: 'app-centre-calne',
  templateUrl: './centre-calne.page.html',
  styleUrls: ['./centre-calne.page.scss'],
})
export class CentreCalnePage implements OnInit {
  selectedDate: { [key: string]: Date } = {}; 
  selectedDay: { [key: string]: string } = {};
  selectedTime: { [key: string]: string } = {};
  timeSlots: any[] = [];
  dateList: Date[] = [];
  activities_calne: any[] = [];
...
// Transfering to Payment page
  ...
  ngOnInit(): void {
    this.firestoreService.getActivitiesCalne().subscribe(
      (data: any) => {
        this.activities_calne= data;
        console.log('Activity:', this.activities_calne);

        // Initialize selectedDate for each activity
        for (const activity of this.activities_calne) {
  ...
  selectDate(date: Date, activity: any) {
    ...
    this.firestoreService
      .getTimeSlotsForCalne(activity.id, this.selectedDay[activity.id])
      .subscribe(
        (timeSlotsData) => {
          if (timeSlotsData) {
            const index = this.activities_calne.findIndex(
              (item) => item.id === activity.id
            );
            if (index !== -1) {
              const timeSlots = Object.values(timeSlotsData);
            // Sorting the time slots in ascending order
            this.activities_calne[index].timeSlots = timeSlots.sort();
            }
          } else {
            const index = this.activities_calne.findIndex(
              (item) => item.id === activity.id
            ...
            if (index !== -1) {
              this.activities_calne[index].timeSlots = [];
            }
          }
        },
        (error) => {
  ...
  bookNow() {
    if (!this.storedSelectedDate || !this.selectedActivityTimeSlot) {
      ...
      this.router.navigate(['payment'], {
        ...
        queryParams: {
          centreName: "Calne Community Campus",
