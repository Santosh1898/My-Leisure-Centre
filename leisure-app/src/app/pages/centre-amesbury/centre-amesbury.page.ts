...
@Component({
  selector: 'app-centre-amesbury',
  templateUrl: './centre-amesbury.page.html',
  styleUrls: ['./centre-amesbury.page.scss'],
})

export class CentreAmesburyPage implements OnInit {
  selectedDate: { [key: string]: Date } = {}; 
  selectedDay: { [key: string]: string } = {};
  selectedTime: { [key: string]: string } = {};
  timeSlots: any[] = [];
  dateList: Date[] = [];
  activities_amesbury: any[] = [];
...
// Transfering to Payment page
  ...
  ngOnInit(): void {
    this.firestoreService.getActivitiesAmesbury().subscribe(
      (data: any) => {
        this.activities_amesbury = data;
        console.log('Activity:', this.activities_amesbury);

        // Initialize selectedDate for each activity
        for (const activity of this.activities_amesbury) {
  ...
  selectDate(date: Date, activity: any) {
    ...
    this.firestoreService
      .getTimeSlotsForAmesbury(activity.id, this.selectedDay[activity.id])
      .subscribe(
        (timeSlotsData) => {
          if (timeSlotsData) {
            const index = this.activities_amesbury.findIndex(
              (item) => item.id === activity.id
            );
            if (index !== -1) {
              const timeSlots = Object.values(timeSlotsData);
            // Sorting the time slots in ascending order
            this.activities_amesbury[index].timeSlots = timeSlots.sort();
            }
          } else {
            const index = this.activities_amesbury.findIndex(
              (item) => item.id === activity.id
            ...
            if (index !== -1) {
              this.activities_amesbury[index].timeSlots = [];
            }
          }
        },
        (error) => {
  ...
  bookNow() {
    if (!this.storedSelectedDate || !this.selectedActivityTimeSlot) {
      ...
      this.router.navigate(['payment'], {
        ...
        queryParams: {
          centreName: "Amesbury Sports Centre",
