...
@Component({
  selector: 'app-centre-durrington',
  templateUrl: './centre-durrington.page.html',
  styleUrls: ['./centre-durrington.page.scss'],
})
export class CentreDurringtonPage implements OnInit {
  selectedDate: { [key: string]: Date } = {}; 
  selectedDay: { [key: string]: string } = {};
  selectedTime: { [key: string]: string } = {};
  timeSlots: any[] = [];
  dateList: Date[] = [];
  activities_durrington: any[] = [];
...
// Transfering to Payment page
  ...
  ngOnInit(): void {
    this.firestoreService.getActivitiesDurrington().subscribe(
      (data: any) => {
        this.activities_durrington = data;
        console.log('Activity:', this.activities_durrington);

        // Initialize selectedDate for each activity
        for (const activity of this.activities_durrington) {
  ...
  selectDate(date: Date, activity: any) {
    ...
    this.firestoreService
      .getTimeSlotsForDurrington(activity.id, this.selectedDay[activity.id])
      .subscribe(
        (timeSlotsData) => {
          if (timeSlotsData) {
            const index = this.activities_durrington.findIndex(
              (item) => item.id === activity.id
            );
            if (index !== -1) {
              const timeSlots = Object.values(timeSlotsData);
            // Sorting the time slots in ascending order
            this.activities_durrington[index].timeSlots = timeSlots.sort();
            }
          } else {
            const index = this.activities_durrington.findIndex(
              (item) => item.id === activity.id
            ...
            if (index !== -1) {
              this.activities_durrington[index].timeSlots = [];
            }
          }
        },
        (error) => {
  ...
  bookNow() {
    if (!this.storedSelectedDate || !this.selectedActivityTimeSlot) {
      ...
      this.router.navigate(['payment'], {
        ...
        queryParams: {
          centreName: "Durrington Swimming and Fitness Centre",
