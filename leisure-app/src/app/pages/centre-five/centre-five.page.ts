...
@Component({
  selector: 'app-centre-five',
  templateUrl: './centre-five.page.html',
  styleUrls: ['./centre-five.page.scss'],
})
export class CentreFivePage implements OnInit {
...
// Transfering to Payment page
  ...
  ngOnInit(): void {
    this.firestoreService.getActivitiesFive().subscribe(
      (data: any) => {
        this.activities_five = data;
        console.log('Activity:', this.activities_five);

        // Initialize selectedDate for each activity
        for (const activity of this.activities_five) {
  ...
  selectDate(date: Date, activity: any) {
    ...
    this.firestoreService
      .getTimeSlotsForFive(activity.id, this.selectedDay[activity.id])
      .subscribe(
        (timeSlotsData) => {
          if (timeSlotsData) {
            const index = this.activities_five.findIndex(
              (item) => item.id === activity.id
            );
            if (index !== -1) {
              const timeSlots = Object.values(timeSlotsData);
            // Sorting the time slots in ascending order
            this.activities_five[index].timeSlots = timeSlots.sort();
            }
          } else {
            const index = this.activities_five.findIndex(
              (item) => item.id === activity.id
            ...
            if (index !== -1) {
              this.activities_five[index].timeSlots = [];
            }
          }
        },
        (error) => {
  ...
  bookNow() {
    if (!this.storedSelectedDate || !this.selectedActivityTimeSlot) {
      ...
      this.router.navigate(['payment'], {
        ...
        queryParams: {
          centreName: "Five Rivers Health and Wellbeing Centre",
