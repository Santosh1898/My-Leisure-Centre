...
@Component({
  selector: 'app-centre-lime-kiln',
  templateUrl: './centre-lime-kiln.page.html',
  styleUrls: ['./centre-lime-kiln.page.scss'],
})
export class CentreLimeKilnPage implements OnInit {
  selectedDate: { [key: string]: Date } = {}; 
  selectedDay: { [key: string]: string } = {};
  selectedTime: { [key: string]: string } = {};
  timeSlots: any[] = [];
  dateList: Date[] = [];
  activities_limeKiln: any[] = [];
...
// Transfering to Payment page
  ...
  ngOnInit(): void {
    this.firestoreService.getActivitieslimeKiln().subscribe(
      (data: any) => {
        this.activities_limeKiln = data;
        console.log('Activity:', this.activities_limeKiln);

        // Initialize selectedDate for each activity
        for (const activity of this.activities_limeKiln) {
  ...
  selectDate(date: Date, activity: any) {
    ...
    this.firestoreService
      .getTimeSlotsForlimeKiln(activity.id, this.selectedDay[activity.id])
      .subscribe(
        (timeSlotsData) => {
          if (timeSlotsData) {
            const index = this.activities_limeKiln.findIndex(
              (item) => item.id === activity.id
            );
            if (index !== -1) {
              const timeSlots = Object.values(timeSlotsData);
            // Sorting the time slots in ascending order
            this.activities_limeKiln[index].timeSlots = timeSlots.sort();
            }
          } else {
            const index = this.activities_limeKiln.findIndex(
              (item) => item.id === activity.id
            ...
            if (index !== -1) {
              this.activities_limeKiln[index].timeSlots = [];
            }
          }
        },
        (error) => {
  ...
  bookNow() {
    if (!this.storedSelectedDate || !this.selectedActivityTimeSlot) {
      ...
      this.router.navigate(['payment'], {
        ...
        queryParams: {
          centreName: "Lime Kiln Leisure Centre",
