...
@Component({
  selector: 'app-centre-activity-zone',
  templateUrl: './centre-activity-zone.page.html',
  styleUrls: ['./centre-activity-zone.page.scss'],
})
export class CentreActivityZonePage implements OnInit {
  selectedDate: { [key: string]: Date } = {}; 
  selectedDay: { [key: string]: string } = {};
  selectedTime: { [key: string]: string } = {};
  timeSlots: any[] = [];
  dateList: Date[] = [];
  activities_activityZone: any[] = [];
...
// Transfering to Payment page
  ...
  ngOnInit(): void {
    this.firestoreService.getActivitiesactivityZone().subscribe(
      (data: any) => {
        this.activities_activityZone = data;
        console.log('Activity:', this.activities_activityZone);

        // Initialize selectedDate for each activity
        for (const activity of this.activities_activityZone) {
  ...
  selectDate(date: Date, activity: any) {
    ...
    this.firestoreService
      .getTimeSlotsForactivityZone(activity.id, this.selectedDay[activity.id])
      .subscribe(
        (timeSlotsData) => {
          if (timeSlotsData) {
            const index = this.activities_activityZone.findIndex(
              (item) => item.id === activity.id
            );
            if (index !== -1) {
              const timeSlots = Object.values(timeSlotsData);
            // Sorting the time slots in ascending order
            this.activities_activityZone[index].timeSlots = timeSlots.sort();
            }
          } else {
            const index = this.activities_activityZone.findIndex(
              (item) => item.id === activity.id
            ...
            if (index !== -1) {
              this.activities_activityZone[index].timeSlots = [];
            }
          }
        },
        (error) => {
