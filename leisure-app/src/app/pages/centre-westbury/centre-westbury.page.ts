...
@Component({
  selector: 'app-centre-westbury',
  templateUrl: './centre-westbury.page.html',
  styleUrls: ['./centre-westbury.page.scss'],
})
export class CentreWestburyPage implements OnInit {
  selectedDate: { [key: string]: Date } = {}; 
  selectedDay: { [key: string]: string } = {};
  selectedTime: { [key: string]: string } = {};
  timeSlots: any[] = [];
  dateList: Date[] = [];
  activities_westbury: any[] = [];
...
// Transfering to Payment page
  ...
  ngOnInit(): void {
    this.firestoreService.getActivitiesWestbury().subscribe(
      (data: any) => {
        this.activities_westbury = data;
        console.log('Activity:', this.activities_westbury);

        // Initialize selectedDate for each activity
        for (const activity of this.activities_westbury) {
  ...
  selectDate(date: Date, activity: any) {
    ...
    this.firestoreService
      .getTimeSlotsForWestbury(activity.id, this.selectedDay[activity.id])
      .subscribe(
        (timeSlotsData) => {
          if (timeSlotsData) {
            const index = this.activities_westbury.findIndex(
              (item) => item.id === activity.id
            );
            if (index !== -1) {
              const timeSlots = Object.values(timeSlotsData);
            // Sorting the time slots in ascending order
            this.activities_westbury[index].timeSlots = timeSlots.sort();
            }
          } else {
            const index = this.activities_westbury.findIndex(
              (item) => item.id === activity.id
            ...
            if (index !== -1) {
              this.activities_westbury[index].timeSlots = [];
            }
          }
        },
        (error) => {
