...
@Component({
  selector: 'app-centre-warminster',
  templateUrl: './centre-warminster.page.html',
  styleUrls: ['./centre-warminster.page.scss'],
})
export class CentreWarminsterPage implements OnInit {
  selectedDate: { [key: string]: Date } = {}; 
  selectedDay: { [key: string]: string } = {};
  selectedTime: { [key: string]: string } = {};
  timeSlots: any[] = [];
  dateList: Date[] = [];
  activities_warminster: any[] = [];
...
// Transfering to Payment page
  ...
  ngOnInit(): void {
    this.firestoreService.getActivitiesWarminster().subscribe(
      (data: any) => {
        this.activities_warminster = data;
        console.log('Activity:', this.activities_warminster);

        // Initialize selectedDate for each activity
        for (const activity of this.activities_warminster) {
  ...
  selectDate(date: Date, activity: any) {
    ...
    this.firestoreService
      .getTimeSlotsForWarminster(activity.id, this.selectedDay[activity.id])
      .subscribe(
        (timeSlotsData) => {
          if (timeSlotsData) {
            const index = this.activities_warminster.findIndex(
              (item) => item.id === activity.id
            );
            if (index !== -1) {
              const timeSlots = Object.values(timeSlotsData);
            // Sorting the time slots in ascending order
            this.activities_warminster[index].timeSlots = timeSlots.sort();
            }
          } else {
            const index = this.activities_warminster.findIndex(
              (item) => item.id === activity.id
            ...
            if (index !== -1) {
              this.activities_warminster[index].timeSlots = [];
            }
          }
        },
        (error) => {
  ...
  bookNow() {
    if (!this.storedSelectedDate || !this.selectedActivityTimeSlot) {
      ...
      this.router.navigate(['payment'], {
        ...
        queryParams: {
          centreName: "Warminster Sports Centre",
