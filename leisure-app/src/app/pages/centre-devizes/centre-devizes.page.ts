...
@Component({
  selector: 'app-centre-devizes',
  templateUrl: './centre-devizes.page.html',
  styleUrls: ['./centre-devizes.page.scss'],
})
export class CentreDevizesPage implements OnInit {
  selectedDate: { [key: string]: Date } = {}; 
  selectedDay: { [key: string]: string } = {};
  selectedTime: { [key: string]: string } = {};
  timeSlots: any[] = [];
  dateList: Date[] = [];
  activities_devizes: any[] = [];
...
// Transfering to Payment page
  ...
  ngOnInit(): void {
    this.firestoreService.getActivitiesDevizes().subscribe(
      (data: any) => {
        this.activities_devizes = data;
        console.log('Activity:', this.activities_devizes);

        // Initialize selectedDate for each activity
        for (const activity of this.activities_devizes) {
  ...
  selectDate(date: Date, activity: any) {
    ...
    this.firestoreService
      .getTimeSlotsForDevizes(activity.id, this.selectedDay[activity.id])
      .subscribe(
        (timeSlotsData) => {
          if (timeSlotsData) {
            const index = this.activities_devizes.findIndex(
              (item) => item.id === activity.id
            );
            if (index !== -1) {
              const timeSlots = Object.values(timeSlotsData);
            // Sorting the time slots in ascending order
            this.activities_devizes[index].timeSlots = timeSlots.sort();
            }
          } else {
            const index = this.activities_devizes.findIndex(
              (item) => item.id === activity.id
            ...
            if (index !== -1) {
              this.activities_devizes[index].timeSlots = [];
            }
          }
        },
        (error) => {
  ...
  bookNow() {
    if (!this.storedSelectedDate || !this.selectedActivityTimeSlot) {
      ...
      this.router.navigate(['payment'], {
        ...
        queryParams: {
          centreName: "Devizes Leisure Centre",
